/*=============================================================================* 
* Pay Decile Analysis - Glass Ceiling in Dutch Organizations
*==============================================================================*
...
*** Settings - run config file
	...
	global adir			"H:/Projects/CJ/segregation/sex/analysis/d_shares"
	...
	foreach year of num 2011/2023 {
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=woman (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = woman if og_sector_alt==`sect', by(withind)
				gen sex="Women"
				gen subpop=`sect'
				order subpop, before(withind)
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=man (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = man if og_sector_alt==`sect', by(withind)
				gen sex="Men"
				gen subpop=`sect'
				order subpop, before(withind)
		...
		*
		...
		preserve
			...
			append using "${adir}/withinq_w_71" "${adir}/withinq_w_72" ///
			"${adir}/withinq_w_81" "${adir}/withinq_w_82" ///
			"${adir}/withinq_w_91" "${adir}/withinq_w_92" ///
			"${adir}/withinq_m_71" "${adir}/withinq_m_72" ///
			"${adir}/withinq_m_81" "${adir}/withinq_m_82" ///
			"${adir}/withinq_m_91" "${adir}/withinq_m_92" ///
			"${adir}/withinq_m_93"
		...
		**************
		foreach lbe of num 1 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=woman (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = woman if lbe==`lbe', by(withind)
				gen sex="Women"
				gen subpop=`lbe'
				order subpop, before(withind)
		...
		*
		foreach lbe of num 1 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=man (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = man if lbe==`lbe', by(withind)
				gen sex="Men"
				gen subpop=`lbe'
				order subpop, before(withind)
		...
		**************
		...
		foreach cohort of num 1/7 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=woman (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = woman if cohort==`cohort', by(withind)
				gen sex="Women"
				gen subpop=`cohort'
				order subpop, before(withind)
		...
		*
		foreach cohort of num 1/7 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=man (median) tot_woman=tot_woman ///
					n_org=n_org (sum) n = man if cohort==`cohort', by(withind)
				gen sex="Men"
				gen subpop=`cohort'
				order subpop, before(withind)
...
* ---------------------------------------------------------------------------- *
	...
	foreach year of num 2011/2023 {
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=woman (sum) n = woman if og_sector_alt==`sect', by(beid withind)
				gegen tot_woman = mean(share_be)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) tot_woman=tot_woman ///
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=man (sum) n = man if og_sector_alt==`sect', by(beid withind)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withind)
				gen sex="Men"
		...
		*
		...
		preserve
			...
			lab def gksbs_lbl 71 "Size: 100-149 employees" ///
		...
		**************
		foreach lbe of num 1 2 3 {
			preserve
				collapse (mean) share_be=woman (sum) n = woman if lbe==`lbe', by(beid withind)
				gegen tot_woman = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_woman=tot_woman ///
		...
		*
		foreach lbe of num 1 2 3 {
			preserve
				collapse (mean) share_be=man (sum) n = man if lbe==`lbe', by(beid withind)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withind)
				gen sex="Men"
		...
		**************
		...
		foreach cohort of num 1/7 {
			preserve
				collapse (mean) share_be=woman (sum) n = woman if cohort==`cohort', by(beid withind)
				gegen tot_woman = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_woman=tot_woman ///
		...
		*
		foreach cohort of num 1/7 {
			preserve
				collapse (mean) share_be=man (sum) n = man if cohort==`cohort', by(beid withind)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withind)
				gen sex="Men"
...
* ---------------------------------------------------------------------------- *
	...
	*Combine in one file
	...
	save "${adir}/withind_sex", replace
		
	export excel using "${adir}/withind_sex", firstrow(variables) replace
