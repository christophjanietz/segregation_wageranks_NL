/*=============================================================================* 
* Pay Quintile Analysis - Ethnic Segregation across Wage Ranks
*==============================================================================*
...
*** Settings - run config file
	...
	global adir			"H:/Projects/CJ/segregation/ethnic/analysis/q_shares"
...
* ---------------------------------------------------------------------------- *
	...
	foreach year of num 2011/2023 {
		...
		merge 1:1 rinpersoon using "${dGBA}/gba_rin_2023", ///
			keep(match) keepusing(rin_miggrp_cbs) nogen 
	...
	foreach year of num 2011/2023 {
		...
		**********************
		preserve
			...
			collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
				n_org=n_org (sum) n = nwstrn, by(withinq)
			drop if withinq==.
		...
		preserve
			...
			collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
				n_org=n_org (sum) n = wstrn, by(withinq)
			drop if withinq==.
			gen wstrn="Western"
		...
		*************
		foreach ind of num 1/19 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if be_industry==`ind', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`ind'
		...
		*
		foreach ind of num 1/19 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if be_industry==`ind', by(withinq)
				gen wstrn="Western"
				gen subpop=`ind'
				order subpop, before(withinq)
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if og_sector_alt==`sect', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`sect'
				order subpop, before(withinq)
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if og_sector_alt==`sect', by(withinq)
				gen wstrn="Western"
				gen subpop=`sect'
				order subpop, before(withinq)
		...
		*********
		foreach gksbs of num 60 71 72 81 82 91 92 93 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if be_gksbs==`gksbs', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`gksbs'
				order subpop, before(withinq)
		...
		*
		foreach gksbs of num 60 71 72 81 82 91 92 93 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if be_gksbs==`gksbs', by(withinq)
				gen wstrn="Western"
				gen subpop=`gksbs'
				order subpop, before(withinq)
		...
		*
		...
		preserve
			...
			append using "${adir}/withinq_nw_60" "${adir}/withinq_nw_71" ///
			"${adir}/withinq_nw_72" "${adir}/withinq_nw_81" ///
			"${adir}/withinq_nw_82" "${adir}/withinq_nw_91" ///
			"${adir}/withinq_nw_92" ///
			"${adir}/withinq_w_60" "${adir}/withinq_w_71" ///
			"${adir}/withinq_w_72" "${adir}/withinq_w_81" ///
			"${adir}/withinq_w_82" "${adir}/withinq_w_91" ///
			"${adir}/withinq_w_92" "${adir}/withinq_w_93"
			...
			lab def gksbs_lbl 60 "Size: 50-99 employees" ///
				71 "Size: 100-149 employees" 72 "Size: 150-199 employees" ///
		...
		***************
		foreach legal of num 43 57 74 900 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if vep_legalform==`legal', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`legal'
				order subpop, before(withinq)
		...
		*
		foreach legal of num 43 57 74 900  {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if vep_legalform==`legal', by(withinq)
				gen wstrn="Western"
				gen subpop=`legal'
				order subpop, before(withinq)
		...
		**************
		foreach owner of num 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if og_ownership==`owner', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`owner'
				order subpop, before(withinq)
		...
		*
		foreach owner of num 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if og_ownership==`owner', by(withinq)
				gen wstrn="Western"
				gen subpop=`owner'
				order subpop, before(withinq)
		...
		**************
		foreach cao of num 0 1 2 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if cao==`cao', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`cao'
				order subpop, before(withinq)
		...
		*
		foreach cao of num 0 1 2 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if cao==`cao', by(withinq)
				gen wstrn="Western"
				gen subpop=`cao'
				order subpop, before(withinq)
		...
		**************
		foreach lbe of num 1 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if lbe==`lbe', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`lbe'
				order subpop, before(withinq)
		...
		*
		foreach lbe of num 1 2 3 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if lbe==`lbe', by(withinq)
				gen wstrn="Western"
				gen subpop=`lbe'
				order subpop, before(withinq)
		...
		**************
		...
		foreach cohort of num 1/7 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=nwstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = nwstrn if cohort==`cohort', by(withinq)
				gen wstrn="Non-Western"
				gen subpop=`cohort'
				order subpop, before(withinq)
		...
		*
		foreach cohort of num 1/7 {
			preserve
				...
				gen n_org = r(J)
				collapse (mean) share=wstrn (median) tot_nwstrn=tot_nwstrn ///
					n_org=n_org (sum) n = wstrn if cohort==`cohort', by(withinq)
				gen wstrn="Western"
				gen subpop=`cohort'
				order subpop, before(withinq)
...
* ---------------------------------------------------------------------------- *
	...
	foreach year of num 2011/2023 {
		...
		**********************
		preserve
			collapse (mean) share_be=nwstrn (sum) n = nwstrn, by(beid withinq)
		...
		restore
		preserve
			collapse (mean) share_be=wstrn (sum) n = wstrn, by(beid withinq)
			gunique beid
			gen n_org=r(J)
		...
		*************
		foreach ind of num 1/19 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if be_industry==`ind', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach ind of num 1/19 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if be_industry==`ind', by(beid withinq)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if og_sector_alt==`sect', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if og_sector_alt==`sect', by(beid withinq)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		*********
		foreach gksbs of num 60 71 72 81 82 91 92 93 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if be_gksbs==`gksbs', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach gksbs of num 60 71 72 81 82 91 92 93 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if be_gksbs==`gksbs', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		***************
		foreach legal of num 43 57 74 900 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if vep_legalform==`legal', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach legal of num 43 57 74 900  {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if vep_legalform==`legal', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		**************
		foreach owner of num 2 3 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if og_ownership==`owner', by(beid withinq)
				gegen tot_nwstrn = mean(share_be) 
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach owner of num 2 3 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if og_ownership==`owner', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		**************
		foreach cao of num 0 1 2 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if cao==`cao', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach cao of num 0 1 2 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if cao==`cao', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		**************
		foreach lbe of num 1 2 3 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if lbe==`lbe', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach lbe of num 1 2 3 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if lbe==`lbe', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
		...
		**************
		...
		foreach cohort of num 1/7 {
			preserve
				collapse (mean) share_be=nwstrn (sum) n = nwstrn if cohort==`cohort', by(beid withinq)
				gegen tot_nwstrn = mean(share_be)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) tot_nwstrn=tot_nwstrn ///
		...
		*
		foreach cohort of num 1/7 {
			preserve
				collapse (mean) share_be=wstrn (sum) n = wstrn if cohort==`cohort', by(beid withinq)
				gunique beid
				gen n_org = r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) n = n, by(withinq)
				gen wstrn="Western"
	...
	* Fill missing of total average share of Non_western across quintiles
...
* ---------------------------------------------------------------------------- *
	...
	*Combine in one file
	...
	save "${adir}/withinq_ethnicity", replace
		
	export excel using "${adir}/withinq_ethnicity", firstrow(variables) replace
