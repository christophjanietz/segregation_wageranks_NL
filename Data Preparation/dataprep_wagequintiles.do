...
*
* - Within-organization sex segregation across wage ranks (quintiles).
*
* NIDIO files used:
...
*** Settings - run config file
	...
	global adir			"H:/Projects/CJ/glass/analysis/q_shares"
...
* ---------------------------------------------------------------------------- *
	...
	foreach year of num 2011/2023 {
		...
		* Remove few missings of wage
		...
		// Generate within-firm wage ranks (quintiles)
		gquantiles withind = hwage, xtile nquantiles(5) by(beid)
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=woman (median) tot_woman=tot_woman ///
					n_org=n_org (sum) N = woman if og_sector==`sect', by(withinq)
				gen sex="Women"
				gen subpop=`sect'
				order subpop, before(withinq)
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				...
				gen n_org=r(J)
				collapse (mean) share=man (median) tot_woman=tot_woman ///
					n_org=n_org (sum) N = man if og_sector==`sect', by(withinq)
				gen sex="Men"
				gen subpop=`sect'
				order subpop, before(withinq)
...
* ---------------------------------------------------------------------------- *
	...
	foreach year of num 2011/2023 {
		...
		merge m:1 beid year using "${wdir}/data/q/analysis_org", ///
		...
		***********
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=woman (sum) N = woman if og_sector==`sect', by(beid withinq)
				gegen tot_woman = mean(share_be)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) tot_woman=tot_woman ///
		...
		*
		foreach sect of num 11 12 13 15 {
			preserve
				collapse (mean) share_be=man (sum) N = man if og_sector==`sect', by(beid withinq)
				gunique beid
				gen n_org=r(J)
				collapse (mean) share=share_be (median) n_org=n_org (sum) N = N, by(withinq)
				gen sex="Men"
...
* ---------------------------------------------------------------------------- *
	...
	* Combine in one file
	...
	export excel using "${adir}/withinq", firstrow(variables)
